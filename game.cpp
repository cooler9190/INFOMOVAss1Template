...
void DrawWuLine( Surface *screen, int X0, int Y0, int X1, int Y1, uint clrLine )
{
    ...
    if (X0 < 0 || X0 >= SCRWIDTH || Y0 < 0 || Y0 >= SCRHEIGHT) return;
    ...
    unsigned int ErrorAdj;
    unsigned int ErrorAccTemp, Weighting;
    ...
    int grayl = (77 * rl + 150 * gl + 29 * bl) >> 8;
    ...
    pixel that X advances each time Y advances 1 pixel, truncating the
        ...
        if (X1 < 0 || X1 >= SCRWIDTH || Y1 < 0 || Y1 >= SCRHEIGHT) return;
